...
class SalesController {
  ...
  updateSalesOrder = async (req: Request, res: Response) => {
    try {
      ...
      const updated = await this.SalesService.updateSales(id, {
        ...req.body,
        updated_by,
      });
  ...
  topSellingCategories = async (req: Request, res: Response) => {
    ...
    if (allSales && allSales.length > 0) {
      ...
      sortedTopCategories
        .slice(0, 10)
        .forEach(([categoryId, quantity], index) => {
          console.log(
            `${
              index + 1
  ...
  deleteSales = async (req: Request, res: Response) => {
    ...
    try {
      if (!id)
        return res
          .status(400)
          .json({ error: true, message: "please supply sale id" });
  ...
  getMonthlySales = async (req: Request, res: Response) => {
    try {
      ...
      monthlySales.forEach((month) => {
        ...
        salesData[monthIndex] = month.count;
      });
